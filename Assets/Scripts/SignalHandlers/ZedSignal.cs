...
namespace DanceGraph
{
    namespace SignalBehaviours
    {
        ...
        public class ZedSignal : MonoBehaviour, sig.ISignalHandler
        {
            ...
            private bool placedInWorld = false;
            ...
            public GameObject LoadAvatar(String name, bool simpleSmoothing = false)
            {
                simplePrediction = simpleSmoothing;
                avatar = (GameObject)Resources.Load("ybotAvatar", typeof(GameObject));
                ...
                GameObject trackedAvatar = skeletonHandler.DanceGraphCreate(avatar, bodyFormat);

                Debug.Log(String.Format("Spawned avatar at {0}", trackedAvatar.transform.position));

                skeletonHandler.AttachNameTag(avatar, name);
            ...
            public void DrivePositionRotationSkeleton(int userID, Vector3 rootPos, Quaternion rootOri, Vector3 [] keypoints, Quaternion [] bones) {
                ...
                skeletonHandler.DanceGraphSetHumanPoseControlKPRot(rootPos, rootOri, keypoints, bones);
                //skeletonHandler.DanceGraphSetHumanPoseControlKP(rootPos, rootOri, keypoints);
            ...
            public void HandlePositionalData(ReadOnlySpan<byte> data, in SignalMetadata sigMeta) {
                ...
                if (!placedInWorld)
                {
                    Debug.Log($"Placing object {gameObject.name}");
            ...
            public void DumpBonePositions() {
                List<Transform> bones = ListChildren(avatar.transform);
                String ss = "Bones: ";
            ...
            public void HandleDualData(ReadOnlySpan<byte> data, in SignalMetadata sigMeta)
            {
                ...
                Transform cTran = avatar.transform.Find("mixamorig:Hips");
                if (!placedInWorld)
                {
                ...
                if (adjustment.rotate180) {
                    keypoints[0].x = -keypoints[0].x + adjustment.delta.x;
                    keypoints[0].z = -keypoints[0].z + adjustment.delta.z;
                    DrivePositionRotationSkeleton(sigMeta.userIdx, root_pos, q * root_ori.quat(), keypoints, bones);
                }
                else {
